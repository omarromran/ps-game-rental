...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (form) form.addEventListener('submit', handleGameFormSubmit);
...
async function initializeData() {
  try {
    const response = await fetch('/api/games/my/games', { headers: authHeaders() });
    ...
    gameInventory = Array.isArray(data.data?.games ? data.data.games : data)
...
async function fetchActivityLog() {
  try {
    const response = await fetch('/api/rentals/store', { headers: authHeaders() });
...
function updateDashboardStats() {
  ...
  if (activeRentalsEl) activeRentalsEl.textContent = gameInventory.filter(g => g.status === 'Rented').length;
  if (totalEarningsEl) {
...
function renderActivityTable(activities) {
  ...
  if (!Array.isArray(activities) || !activities.length) {
    activityBody.innerHTML = '<tr><td colspan="4" style="text-align:center;">No recent rental history</td></tr>';
    return;
  }
  ...
  activityBody.innerHTML = activities.slice(0, 10).map(act => {
    ...
    const date = act.createdAt
      ? new Date(act.createdAt).toLocaleDateString()
      : (act.startDate ? new Date(act.startDate).toLocaleDateString() : 'N/A');
    return `
      <tr>
        <td>${gameTitle}</td>
        <td>${customerName}</td>
...
async function handleGameFormSubmit(e) {
  ...
  if (!title) { showFormError('Game title is required.'); return; }
  if (title.length < 2) { showFormError('Game title must be at least 2 characters.'); return; }
  if (!platform) { showFormError('Please select a platform.'); return; }
  if (!category) { showFormError('Please select a genre.'); return; }
  if (!pricePerDay) { showFormError('Daily price is required.'); return; }
  if (isNaN(pricePerDay) || parseFloat(pricePerDay) <= 0) { showFormError('Price must be a positive number.'); return; }
  if (parseFloat(pricePerDay) > 10000) { showFormError('Price cannot exceed 10,000 EGP.'); return; }
  if (!currentEditGameId && files.length === 0) { showFormError('Please upload at least one image for the game.'); return; }
  ...
  try {
    ...
    if (response.ok) {
      if (currentEditGameId) {
        ...
        gameInventory = gameInventory.map(game => game._id === updatedGame._id ? updatedGame : game);
        cancelGameEdit();
      } else {
    ...
    showFormError(error.message || 'Error connecting to server.');
  }
}
